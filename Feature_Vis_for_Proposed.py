...
class CustoMultiModalImgaeTextClassification(torch.nn.Module):
    def __init__(self, num_labels=3, image_model=None, text_model=None):
        ...
        self.dropout = torch.nn.Dropout(0.1)
        self.softmax = torch.nn.Softmax(dim=1)
    def forward(self, pixel_values, input_ids, attention_mask, image_image=None,laebel=None):
        ...
        avg_fusion_np = cross_text_to_image_attn.cpu().detach().numpy()
        ...
        plt.title('Feature Distribution of Text-Image Cross Attention Fusion')
        plt.xlabel('Feature Values')
        plt.ylabel('Frequency')
        plt.savefig(f'./cross_attention_fusion{image_image}.png', dpi=500, bbox_inches='tight')
        plt.close()

        print(f"Flattened features shape: {flattened_features.shape} for image: {image_image}")
        output = self.linear(output)
        output = self.softmax(output)
        print(f"Label: {laebel} Softmax output: {output}")
        print(f"Mean of features: {mean}, Var: {var}, Standard Deviation of features: {std_dev}")
        return output, flattened_features


def TestMMTC():
    best_model_path = './MulMoSenT_Model/Proposed_lr_1.pt'
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            output, emeded = model(images, input_ids, attention_mask,name,labels)
            output = output.argmax(dim=1).cpu().numpy()
            labels = labels.cpu().numpy()
            predict_labels.extend(output)
            actual_labels.append(labels[0])
            emeddings_list.append(emeded)
