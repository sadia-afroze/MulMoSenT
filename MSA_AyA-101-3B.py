...
class CustomDataLoaderMMTC(Dataset):
    def __init__(self, csv_file):
        ...
        self.tokenizer = AutoTokenizer.from_pretrained("bigscience/bloom-1b",use_auth_token="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")
...
# Multi-modal Model with BLOOM and Vision Encoder
class MultiModalModel(nn.Module):
    def __init__(self, num_labels=3):
        super(MultiModalModel, self).__init__()
        self.text_model = AutoModelForCausalLM.from_pretrained("bigscience/bloom-1b", return_dict=True, output_hidden_states=True, use_auth_token="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")
    ...
    def forward(self, input_ids, attention_mask):
        ...
        cls_embedding = torch.mean(hidden_states, dim=1)  # Average pooling
...
def test_mmtc(temp_model):
    # Load test dataset
    dataset = CustomDataLoaderMMTC("./test.csv")
    dataloader = DataLoader(dataset, batch_size=24, shuffle=False)

    # Initialize model and load trained weights
    model = temp_model #MultiModalModel(num_labels=3).to(device)
    #model_dict = torch.load("./Baseline-Models/BLOOM.pth")
...
def train_mmtc():
    ...
    for epoch in range(20):
        ...
        if global_acc < acc:
            global_acc = acc
            torch.save(model.state_dict(), "./BLOOM.pth")
