...
from transformers import ViTModel, ElectraModel, ElectraTokenizer, get_linear_schedule_with_warmup, XLMRobertaModel, XLMRobertaTokenizer
...
def TestMMTC():
    best_model_path = './Ablation_Model/MSA_XMLRoBERTa_ViT_mfb_Focal.pt'
...
def TrainMMTC():
    ...
    dataloader = DataLoader(datasets, batch_size=18, shuffle=True)
    num_classes = 3
    ...
    for epoch in range(epochs):
        ...
        if acc > globAlaccuracy:
            globAlaccuracy = acc
            best_model = model
            torch.save(best_model, "./Ablation_Model/MSA_XMLRoBERTa_ViT_mfb_Focal.pt")
