...
token = "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"  # Replace with your actual token
...
class CustomImageClassification(torch.nn.Module):
    def __init__(self, num_labels=3):
        ...
        self.num_labels = 3
        self.model = ViTModel.from_pretrained("google/vit-base-patch16-224-in21k", return_dict=True, use_auth_token=token)
...
class CustomTextClassification(torch.nn.Module):
    def __init__(self, num_labels=3):
        ...
        self.model = ElectraModel.from_pretrained("csebuetnlp/banglabert", return_dict=True,use_auth_token=token)
        self.linear = torch.nn.Linear(self.hidden_size, self.num_labels)
        self.activation = torch.nn.ReLU()
...
class CustoMultiModalImgaeTextClassification(torch.nn.Module):
    def __init__(self, num_labels=3, image_model=None, text_model=None):
        ...
        self.linear = torch.nn.Linear(self.embed_dim, self.num_labels)
        self.activation = torch.nn.ReLU()
        self.dropout = torch.nn.Dropout(0.1)
        self.softmax = torch.nn.Softmax(dim=1)
    def forward(self, pixel_values, input_ids, attention_mask,image_image, labels=None):
        ...
        avg_fusion = (image_output + text_output) / 2.
        avg_fusion_np = avg_fusion.cpu().detach().numpy()
        flattened_features = avg_fusion_np.flatten()
        # Calculate mean and standard deviation
        mean = np.mean(flattened_features)
        std_dev = np.std(flattened_features)
        # Plot the distribution
        mean = np.mean(flattened_features)
        var = np.var(flattened_features)
        std_dev = np.std(flattened_features)
        plt.figure(figsize=(10, 6))
        sns.histplot(flattened_features, bins=50, kde=True, color='blue', alpha=0.6, edgecolor='black')
        plt.title('Feature Distribution of Average Fusion')
        plt.xlabel('Feature Values')
        plt.ylabel('Frequency')
        plt.savefig(f'./avg_fusion{image_image}.png', dpi=500, bbox_inches='tight')
        plt.close()
        print(f"Flattened features shape: {flattened_features.shape} for image: {image_image}")
        output = self.linear(avg_fusion)
        output = self.softmax(output)
        print(f"Label: {image_image} Softmax output: {output}")
        print(f"Mean of features: {mean}, Var: {var}, Standard Deviation of features: {std_dev}")
        return output, avg_fusion_np

def TestMMTC():
    best_model_path = './MulMoSenT_Model/Proposed_avg_lr_1.pt'
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            #last part of image path
            name = os.path.basename(image_path[0])

            text_array = batch['text']
            output, avg_fusion = model(images, input_ids, attention_mask, name,labels)
            ...
            embeddings_list.append(avg_fusion)
...
#    with open('./BMSA_Error_Sample_for_avg.csv', 'w') as f:
...
def TrainMMTC():
    ...
    dataloader = DataLoader(datasets, batch_size=20, shuffle=True) # 32
    num_classes = 3
    ...
    for epoch in range(epochs):
        ...
        if acc > globAlaccuracy:
            globAlaccuracy = acc
            best_model = model
            torch.save(best_model, "./MulMoSenT_Model/Proposed_avg_lr_1.pt")
