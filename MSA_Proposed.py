...
def TestMMTC(test_model):
    ...
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    print(device)
    model =  test_model #torch.load('./Model/BMMTC_Focal_loss_Image_Text_MFB_lr2e-5_1000dim_Fold4.pt').to(device)
    model.eval()
    predict_labels = []
    actual_labels = []
    Image_path_list = []
...
def TrainMMTC():
    ...
    for epoch in range(epochs):
        ...
        if acc > globAlaccuracy:
            globAlaccuracy = acc
            best_model = model
            torch.save(best_model, "./Model/BMMTC_Focal_loss_Image_Text_MFB_lr2e-5_1000dim_Fold5.pt")
